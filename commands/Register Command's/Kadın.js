...
kdb.push(`kullanici.${user.id}.kayıt`, { isim: isim, yas: yas ? `| ${yas}` : ``, rol: "Kadın",   });
...
client.channels.cache.find(a => a.name === "cmd-log").send(`[\`${moment(+Date.now()).format(`Do MMMM YYYY | HH:mm`)}\`] - \`${message.author.tag}\` adlı kullanıcı <#${message.channel.id}> kanalında \`.kadın\` komutunu kullandı. [\`${message.content}\`]`);
        ...
        ${user} kullanıcısı **Kadın** olarak kayıt edildi`))
...
exports.conf = { command: "kadın",  description: "Belirtilen kişiyi erkek olarak kayıt eder",  aliases: ["k", "woman", "register"]}